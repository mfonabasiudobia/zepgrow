...
const AdListing = () => {
  ...
  const getLocationWithMap = async (pos) => {
    try {
      ...
      // Update stores based on geocoding result
      if (country) {
        const countryMatch = CountryStore.Countries.find(c => c.name === country);
        if (countryMatch) {
          setCountryStore(prev => ({
            ...prev,
            SelectedCountry: countryMatch
          }));
          await getStatesData('', 1);
        }
      }
    } catch (error) {
      console.error('Error fetching location data:', error);
      toast.error(t('errorFetchingLocation'));
    }
  }
  ...
  // Add missing search handlers
  const handleCountrySearch = (value) => {
    setCountryStore(prev => ({
      ...prev,
      CountrySearch: value,
      currentPage: 1,
      Countries: [] // Reset countries when searching
    }));
  };
  ...
  const handleStateSearch = (value) => {
    setStateStore(prev => ({
      ...
      StateSearch: value,
      currentPage: 1,
      States: [] // Reset states when searching
    }));
  };
  ...
  const handleCitySearch = (value) => {
    setCityStore(prev => ({
      ...
      CitySearch: value,
      currentPage: 1,
      Cities: [] // Reset cities when searching
    }));
  };
  ...
  const handleAreaSearch = (value) => {
    setAreaStore(prev => ({
      ...
      AreaSearch: value,
      currentPage: 1,
      Areas: [] // Reset areas when searching
    }));
  };

  // Add effect for state search
  useEffect(() => {
    if (CountryStore?.SelectedCountry?.id) {
      const timeout = setTimeout(() => {
        getStatesData(StateStore?.StateSearch, 1);
  ...
  }, [StateStore?.StateSearch, CountryStore?.SelectedCountry?.id]);

  // Add effect for city search
  useEffect(() => {
    if (StateStore?.SelectedState?.id) {
      const timeout = setTimeout(() => {
        getCitiesData(CityStore?.CitySearch, 1);
  ...
  }, [CityStore?.CitySearch, StateStore?.SelectedState?.id]);

  // Add effect for area search
  useEffect(() => {
    if (CityStore?.SelectedCity?.id) {
      const timeout = setTimeout(() => {
  ...
  }, [AreaStore?.AreaSearch, CityStore?.SelectedCity?.id]);
