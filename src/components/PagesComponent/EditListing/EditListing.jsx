...
import { generateSlug, isPdf, isValidURL, t, isLogin } from '@/utils'
import { editItemApi, getAreasApi, getCitiesApi, getCoutriesApi, getCustomFieldsApi, getStatesApi, getParentCategoriesApi, getMyItemsApi } from '@/utils/api'
    ...
    useEffect(() => {
        const checkOwnership = async () => {
            try {
                ...
                const adData = res?.data?.data?.[0];
                
                // If no data found or user doesn't own the ad, redirect to home
                ...
                setAdListingDetails({
                    title: adData.name,
                    slug: adData.slug,
                    desc: adData.description,
                    price: adData.price,
                    discounted_price: adData.discounted_price,
                    phonenumber: adData.contact,
                    link: adData.video_link
                });
                
                // Set other form data as needed
                setCurrenCategory(adData.category);
                setCustomFields(adData.custom_fields);
                setLocation(adData.location);
                setUploadedImages(adData.image ? [adData.image] : []);
                setOtherImages(adData.gallery_images);
                setAllCategoryIds(adData.all_category_ids);
                setSelectedCateId(adData.category_id);
                await setEditCategory(adData.all_category_ids);
                setExtraFieldValue(adData.custom_fields);
            } catch (error) {
                console.log(error);
                router.push('/');
            }
