...
const FilterTree = ({ slug, show, setShow, setCategoryIds }) => {
    ...
    const fetchMoreCategory = async () => {
        ...
        try {
            ...
            // Filter out duplicates by slug/key before updating the tree
            const existingKeys = new Set();
            const collectExistingKeys = (nodes) => {
                if (!nodes) return;
                nodes.forEach(node => {
                    if (node.key) existingKeys.add(node.key);
                    if (node.children) collectExistingKeys(node.children);
                });
            };
            
            // Collect all existing keys first
            collectExistingKeys(treeData);
            
            // Filter the new tree data to only include non-duplicates
            const uniqueNewTreeData = newTreeData.filter(node => !existingKeys.has(node.key));

            const updatedData = updateTreeData(treeData, 'all-categories', uniqueNewTreeData);
