...
const ERROR_CODES = {
  ...
  'auth/operation-not-allowed': t('operationnotallowyutubeed'),
...
export const loadGoogleMaps = () => {
  let settings = store.getState()?.Settings?.data?.data;
  const apiKey = settings?.place_api_key || process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY;
  
  if (!apiKey) {
    console.error('Google Maps API key is missing. Please check your settings or environment variables.');
    return { isLoaded: false, loadError: 'API key missing' };
  }
...
export const getnotallowyutubeVideoId = (url) => {
